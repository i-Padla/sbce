...
class InstanceObject extends Instance {
  prepare() {
    ...
    if (isSet(schemaProperties)) {
      Object.keys(schemaProperties).forEach((key) => {
        ...
        const optionsDeactivateNonRequired = this.jedison.getOption(
          'deactivateNonRequired'
        )
        const deactivateNonRequired = getSchemaXOption(
          this.schema,
          'deactivateNonRequired'
        )
        const schemaDeactivateNonRequired = getSchemaXOption(
          schema,
          'deactivateNonRequired'
        )
        ...
        if (
          !isReq &&
          isSet(optionsDeactivateNonRequired) &&
          optionsDeactivateNonRequired === true
        ) {
        ...
        if (
          !isReq &&
          isSet(schemaDeactivateNonRequired) &&
          schemaDeactivateNonRequired === true
        ) {
          musstCreateChild = false
        }
        ...
        if (musstCreateChild) {
          this.createChild(
            schema,
            key,
            hasOwn(initialValue, key) ? initialValue[key] : undefined
          )
        }
      })
    }
    ...
    if (
      isSet(schemaRequired) &&
      this.jedison.isEditor &&
      this.jedison.getOption('enforceRequired') === true
    ) {
      schemaRequired.forEach((requiredProperty) => {
        ...
        if (!hasOwn(this.properties, requiredProperty)) {
          ...
          this.createChild(
            {},
            requiredProperty,
            hasOwn(initialValue, requiredProperty)
              ? initialValue[requiredProperty]
              : undefined
          )
        }
      })
  ...
  removeNotListedPropertiesFromValue(value) {
    const schemaEnforceAdditionalProperties = getSchemaXOption(
      this.schema,
      'enforceAdditionalProperties'
    )
    const enforceAdditionalProperties = isSet(schemaEnforceAdditionalProperties)
      ? schemaEnforceAdditionalProperties
      : this.jedison.getOption('enforceAdditionalProperties')
    ...
    if (
      this.jedison.isEditor &&
      enforceAdditionalProperties &&
      isSet(schemaAdditionalProperties) &&
      schemaAdditionalProperties === false
    ) {
      const compiledPatterns = Object.keys(schemaPatternProperties).map(
        (p) => new RegExp(p)
      )
      Object.keys(value).forEach((propertyName) => {
        const matchesPattern = compiledPatterns.some((re) =>
  ...
  addMissingRequiredPropertiesToValue(value) {
    const enforceRequired =
      getSchemaXOption(this.schema, 'enforceRequired') ??
  ...
  isRequired(property) {
    const schemaRequired = getSchemaRequired(this.schema)
    const inSchemaRequired =
      isSet(schemaRequired) && schemaRequired.includes(property)
  ...
  //     const propertyOrderA =
  //       isParentMap && isNumber(parentOrderMap[keyA])
  //         ? parentOrderMap[keyA]
  //         : getSchemaXOption(a.schema, "propertyOrder");
  //     const propertyOrderB =
  //       isParentMap && isNumber(parentOrderMap[keyB])
  //         ? parentOrderMap[keyB]
  //         : getSchemaXOption(b.schema, "propertyOrder");
  ...
  // New sorting funcion to sort by:
  // Priority 1: per-property x-propertyOrder number
  // Priority 2: parent x-propertyOrder map

...
class ThemeBootstrap5 extends Theme {
  ...
  _adaptHorizontalControl(control, labelCol, inputCol) {
    if (!control.label || control.label.classList.contains("col-form-label"))
      return;
    const lc = labelCol ?? 3;
    const ic = inputCol ?? 6;
    control.container.classList.add("row");
    control.label.classList.add(
      "col-form-label",
      "text-sm-end",
      `col-sm-${lc}`,
    );
  ...
  adaptForHorizontalCheckboxControl(control, labelCol, inputCol) {
    ...
    Array.from(control.container.children).forEach((el) =>
  ...
  getCheckboxControl(config) {
    ...
    const { container, formGroup, input, label, info, description, messages } =
      control;
    container.classList.add("mb-3");
  ...
  getSwitcherModal(config) {
    ...
    control.dialogBody.classList.add(
      "btn-group",
      "btn-group-vertical",
      "w-100",
    );
